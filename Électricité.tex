...
\begin{document}
    ...
    P. instant, moy/active, apparente

    Puissance instantanée : varie en fonction du temps. On choisit $\varphi_i = 0$ et $\varphi_u \neq 0$.\\
    $P =  U_{eff} I_{eff} e^{j\varphi_u} + U_{eff} I_{eff} e^{j(2\omega t + \varphi_u)}$
    $p(t) =  U_{eff} I_{eff} e^{j\varphi_u} + U_{eff} I_{eff} e^{j(2\omega t + \varphi_u)}$

    -> puissance moyenne (ou active) = terme qui ne dépend pas du temps
    Puissance apparente : $U_{eff} I_{eff}$
    ...
    $\varphi_u = \varphi_i$ donc $p(t) = U_{eff} I_{eff} + U_{eff} I_{eff} \cos(2 \omega t) \ge 0$

    \subsection{Condensateur}\label{subsec:condensateur-2}

    $\varphi_u - \varphi_i = - \dfrac{\pi}{2}$ donc $p(t) = 0 + U_{eff} I_{eff} \cos\left(2 \omega t - \frac{\pi}{2}\right)$
    ...
    \section{Transfert d'énergie - adaptation d'impedance}\label{sec:transfert-d'energie---adaptation-d'impedance}
