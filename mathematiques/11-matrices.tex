	...
	\subsection{Définition}
		...
		\twoCol {
			...
			\begin{itemize}
				...
				\item Diagonale : $\forall i = j, a_{i,j} = 0$
...
\section{Inversion de matrices}
	...
	\twoCol[50]{
		...
		Pour une matrice carrée :\\ $A \in \mathcal{M}_n$ est inversible $\iff \rg(A) = n$.\\
		Pour une matrice rectangulaire :\\ $A \in \mathcal{M}_{n,p}$, $\rg(A) \le \min(n,p)$.
	}{
		\section{Transposition}
		Transposée de $A \in \mathcal{M}_{n,p}$ : $B \in \mathcal{M}_{p,n}$ tel que $a_{i,j} = b_{j,i}$.\vspace{5pt}\\
		$^t(AB) = {}^tB \times {}^tA$ ; $\rg({}^tA) = \rg(A)$ ; $({}^tA)^{-1} = {}^tA^{-1}$.\\

		\underline{Matrice symétrique :} $A \in \mathcal{S}_n \iff {}^tA = A$.\\
		\underline{Matrice antisymétrique :} $A \in \mathcal{A}_n \iff {}^tA = -A$.\\

		$\mathcal{S}_n \oplus \mathcal{A}_n$, $\dim(\mathcal{S}_n) = \dfrac{n(n+1)}{2}$ et $\dim(\mathcal{A}_n) = \dfrac{n(n-1)}{2}$.
